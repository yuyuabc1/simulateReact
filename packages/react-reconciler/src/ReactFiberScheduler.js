...
function scheduleCallbackWithExpirationTime(
  ...
  // date().now() - 通过originalStartTimeMs方法获取在没有任何方法执行的时候获取的一个原始时间戳
...
function requestCurrentTime() {
  ...
  /**
   * 调度程序调用requestCurrentTime方法来计算过期时间。过期时间是通过添加到当前时间（开始时间）来计算的。
   * 但是，如果同时有两个更新被安排在同一个事件内，我们应该把他们的开始时间看作是同时的，即使实际到达时间在第一次和第二次之前。
   * 
   * 换句话说，因为过期时间决定了如何批量更新，所以我们希望在同一事件内的具有相同优先级的所有更新都接收相同的过期事件。否则我们会哭的。
   * 
   * 我们持续跟踪两个不同的时间：当前的“渲染器”时间和当前的“调度器”时间。渲染器时间可以随时更新；它只存在于使用最少性能的情况下。
   * 
   * 但是只有在未完成的工作中，或者我们确定它没有在事件中，“调度器”时间才会更新。
   */
